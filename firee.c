...
int add_a_rule(void) {
    ...
    mm_segment_t fs;
    loff_t pos;
    char buffer[1000],store_element[20],*p;
    int i,j=0,k=0; unsigned int array[10];
    int xx=0,jj=0;char main_store[50];
    fp = filp_open("firee.txt",O_RDWR|O_CREAT,0644);  //open
    if (IS_ERR(fp)){   /*handle*/ printk ("open file error \n"); return 0; }
    fs = get_fs();  //get current segment
    set_fs(KERNEL_DS);  // set kernel segment
    pos = 0;
    vfs_read(fp, buffer, sizeof(buffer), &pos);  //read
    filp_close(fp, NULL); //close
    set_fs (fs); // set saved segment
    while(1){
        if(buffer[xx]=='\0'){break;}
        if(buffer[xx]!='\n'){ main_store[jj]=buffer[xx]; jj++;}
        if(buffer[xx]=='\n'){
            for(i=0;i<strlen(main_store);i++){
                if(main_store[i]==','||i==strlen(main_store)-1){
                    array[k]= simple_strtol(store_element,&p,0);
                    k++;j=0;
	            memset(store_element,'\0',20);
                    continue;
                }
                store_element[j]=main_store[i];
                j++;
            }
            a_rule = kmalloc(sizeof(*a_rule), GFP_KERNEL);
            if(a_rule == NULL){printk("error: cannot allocate memory for a_new_rule\n");return 0;}
            a_rule->in_out        = array[0];
            a_rule->src_ip        = array[1];
            a_rule->src_port      = array[2];
            a_rule->dest_ip       = array[3];
            a_rule->dest_port     = array[4];
            a_rule->proto         = array[5];
            a_rule->action        = array[6];
            printk("add_a_rule : in_out=%u, src_ip=%u, src_port=%u, dest_ip=%u, dest_port=%u, proto=%u, action=%u\n", a_rule->in_out, a_rule->src_ip, a_rule->src_port, a_rule->dest_ip, a_rule->dest_port, a_rule->proto, a_rule->action);
            printk("source ip : ");
            IpConverter(a_rule->src_ip);
            printk("destination ip : ");
            IpConverter(a_rule->dest_ip);
